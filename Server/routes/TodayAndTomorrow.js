...
router.post("/get_today_tomorrow", async (req, res) => {
  try {
    ...
    res.json({
      statusCode: 200,
      data: data,
      TodayAndTommorowDataCount: count,
...
router.post("/search_today_tomorrow", async (req, res) => {
  try {
    ...
    var data = await TodayAndTomorrow.find({
      $or: newArray,
    });

    var count = await TodayAndTomorrow.countDocuments({
      $or: newArray,
    });
    ...
    if (categoryType === "today") {
      ...
      try {
        ...
        if (data.length > 0) {
          ...
          // Sort the items based on the addDate field in descending order
          const sortedItems = categoryData.items.sort((a, b) => {
            return new Date(b.addDate) - new Date(a.addDate);
          });

          // Update the categoryData with the sorted items
          categoryData.items = sortedItems;
      ...
      try {
        ...
        // Update the result with the sorted items
