...
router.post("/search_ds_item", async (req, res) => {
  try {
    ...
    var count = await DynamicSection_Item.countDocuments({
      $or: newArray,
    });
    // data.reverse();
    res.json({
...
router.get("/dynamicdata/:ds_category", async (req, res) => {
  try {
    ds_category = req.params.ds_category
    var data = await DynamicSection_Item.find({ds_category})
    res.json({
      data: data,
      statusCode: 200,
      message: "Read All Dynamic Data",
    });
  } catch (error) {
    res.json({
      statusCode: 500,
...
router.get("/ds_item", async (req, res) => {
  try {
    ...
    // Iterate through the data and group items by category
    data.forEach((item) => {
      const { ds_category, ds_itemImage, languageName, isVideo, addDate } = item;
      if (!itemsByCategory[ds_category]) {
        itemsByCategory[ds_category] = [];
      }
      itemsByCategory[ds_category].push({
        ...
        languageName: languageName,
        isVideo: isVideo,
        addDate: new Date(addDate), // Convert addDate string to Date object
      });
    });
    ...
    // Create an array of objects with sorted category and items
    const result = sortedCategories.map((category) => {
      // Sort the items within each category based on the addDate field in descending order
