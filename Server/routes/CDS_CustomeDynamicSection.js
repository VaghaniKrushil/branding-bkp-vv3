...
router.post("/cds/search", async (req, res) => {
  try {
    let newArray = [];
    newArray.push({
    ...
    var data = await CDS_CustomeDynamicSection.find({ $or: newArray }).select('-cds_template');

    var count = await CDS_CustomeDynamicSection.countDocuments({ $or: newArray });
