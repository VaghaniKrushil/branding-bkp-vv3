...
router.get("/banner/custome", async (req, res) => {
  try {
    ...
    data.forEach((element) => {
      const shedualStartDate = new Date(element.shedualStartDate);
      const shedualEndDate = new Date(element.shedualEndDate);
      shedualEndDate.setDate(shedualEndDate.getDate() + 1); // Increment by one day

      if (
        element.shedualSwitch &&
        currentDate >= shedualStartDate &&
        currentDate < shedualEndDate
      ) {
        outputresult.push(element);
      } else if (element.cds_bannerSwitch) {
        outputresult.push(element);
      }
    });

    if (outputresult.length === 0) {
      // If no banners are available, return the default banner with the modified key
      const modifiedDefaultBanner = {
        ...defaultBanner.toObject(),
        cds_bannerImage: defaultBanner.defaultBannerImage,
      };
