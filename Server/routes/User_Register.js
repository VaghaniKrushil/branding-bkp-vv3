...
// Live and Working api current Comment because ios Testing
...
//     try {
//       // const apiUrl = `http://193.46.243.10/app/smsapi/index.php?key=46538AFE27083E&campaign=13444&routeid=3&type=text&contacts=${user.mobileNumber}&senderid=BRAPRO&msg=%5B%23%5D%20DO%20NOT%20SHARE%20%3A%20${otp}%20is%20the%20login%20OTP%20for%20your%20Branding%20profitable%20account.%20Keep%20this%20OTP%20to%20yourself%20for%20account%20safety.&template_id=1707170107181759475&pe_id=1701169787222711884`;
//       const apiUrl = `http://193.46.243.10/app/smsapi/index.php?key=46538AFE27083E&campaign=13444&routeid=3&type=text&contacts=${user.mobileNumber}&senderid=BRAPRO&msg=%5B%23%5D%20DO%20NOT%20SHARE%20%3A%20${otp}%20is%20the%20login%20OTP%20for%20your%20Branding%20profitable%20account.%20Keep%20this%20OTP%20to%20yourself%20for%20account%20safety.&template_id=1707170107181759475&pe_id=1701169787222711884`;
...
router.post("/sendotp", async (req, res) => {
  try {
    ...
    // Convert user.mobileNumber to a string for comparison
    const mobileNumberString = user.mobileNumber.toString();

    // Check if the mobile number is the one you want to restrict (e.g., '9106636361')
    if (mobileNumberString === "9687870025") {
      // If the mobile number matches, set a specific OTP (e.g., 1234)
      user.otp = 3738;
    } else {
      // If the mobile number doesn't match, generate a random 4-digit OTP
    ...
    try {
      const apiUrl = `http://193.46.243.10/app/smsapi/index.php?key=46538AFE27083E&campaign=13444&routeid=3&type=text&contacts=${user.mobileNumber}&senderid=BRAPRO&msg=%5B%23%5D%20DO%20NOT%20SHARE%20%3A%20${user.otp}%20is%20the%20login%20OTP%20for%20your%20Branding%20profitable%20account.%20Keep%20this%20OTP%20to%20yourself%20for%20account%20safety.&template_id=1707170107181759475&pe_id=1701169787222711884`;
...
router.get("/directresponce/:mobileNumber", async (req, res) => {
  try {
    ...
    if (dynamicAuthorization) {
      ...
      if (phpApiResponse.status === 200) {
        const sponsor = phpApiResponse.data.sponsor; // Extract 'sponsor' data from the response
        res.status(200).json({
          statusCode: 200,
  ...
  try {
    ...
    if (!isNaN(searchString)) {
      ...
      newArray.push(
        ...
        {
          adress: { $regex: searchString, $options: "i" },
        }
      );
    }
