...
export default function TodayCreditTransactions() {
  let navigate = useNavigate();
  ...
  const getData = async () => {
    // const token = cookies.get("token");
    try {
      const res = await axios.get("https://brandingprofitable-29d465d7c7b1.herokuapp.com/mlm/todaycredit", {
        params: {
      ...
      setCountData(res.data.todayodayCreditCount); // Make sure to adjust the key here
    } catch (error) {
