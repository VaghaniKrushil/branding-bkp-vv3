  ...
  return (
    <React.Fragment>
      ...
      >
        ...
        <TableCell
          align="center"
          onClick={() => navigate(`/sponcor-details/${row.user_number}/${row.fullName}`)}
...
export default function SponcorPrize() {
  let navigate = useNavigate();

  const [image, setImage] = useState("");
  const [imgLoader, setImgLoader] = useState(false);

  const [sponsorPrizeDebit, setSponsorPrizeDebit] = useState([])
  ...
  const getData = async () => {
    // const token = cookies.get("token");
    try {
      const res = await axios.get("https://brandingprofitable-29d465d7c7b1.herokuapp.com/mlm/total_sponsor", {
        params: {
      ...
      setLoader(false);
      setSponsorPrizeDebit(res.data.history);
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelected = sponsorPrizeDebit?.map((n) => n._id);
  ...
  let handleSearchData = async (values) => {
    const token = cookies.get("token");
    let res = await axios.post(
      "https://brandingprofitable-29d465d7c7b1.herokuapp.com/mlm/search_total_sponsor",
      {
        search: values,
      },
      {
          ...
          {loader ? (
            ...
            <TableContainer>
              <Table aria-label="collapsible table">
                  ...
                  <TableRow>
                    ...
                    {/* <TableCell align="center" padding="checkbox">
                      <Checkbox
                        ...
                        indeterminate={
                          selected.length > 0 &&
                          selected.length < sponsorPrizeDebit?.length
                        }
                        checked={
                          sponsorPrizeDebit?.length > 0 &&
                          selected.length === sponsorPrizeDebit?.length
                        }
                ...
                <TableBody>
                  {sponsorPrizeDebit?.map((row, index) => {
