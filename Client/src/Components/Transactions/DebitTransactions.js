  ...
  return (
    <React.Fragment>
      ...
      >
        ...
        <TableCell
          align="center"
          onClick={() => navigate(`/alldebit-history/${row.user_number}/${row.fullName}`)}
...
export default function DebitTransactions() {
  ...
  const getData = async () => {
    // const token = cookies.get("token");
    try {
      const res = await axios.get("https://brandingprofitable-29d465d7c7b1.herokuapp.com/mlm/total_debit", {
        params: {
  ...
  let handleSearchData = async (values) => {
    let res = await axios.post("https://brandingprofitable-29d465d7c7b1.herokuapp.com/mlm/search_total_debit", {
      search: values,
    });
    if (res.data.statusCode === 200) {
      if (values !== "") {
