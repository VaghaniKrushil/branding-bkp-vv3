  ...
  return (
    <React.Fragment>
      ...
      >
        ...
        <TableCell align="center">{row.status ? "Paid" : "Unpaid"}</TableCell>
...
export default function CreditTransactions() {
  ...
  const getData = async () => {
    // const token = cookies.get("token");
    try {
      const res = await axios.get("https://brandingprofitable-29d465d7c7b1.herokuapp.com/mlm/totalcredit", {
        params: {
      ...
      setCountData(res.data.totalCreditCount); // Make sure to adjust the key here
    } catch (error) {
  ...
  let handleSearchData = async (values) => {
    ...
    let res = await axios.post(
      "https://brandingprofitable-29d465d7c7b1.herokuapp.com/mlm/totalcredit/search",
      {
        search: values,
      },
    );
