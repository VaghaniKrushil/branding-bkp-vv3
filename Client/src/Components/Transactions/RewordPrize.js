...
export default function RewordPrize() {
  let navigate = useNavigate();
  ...
  const getData = async () => {
    // const token = cookies.get("token");
    try {
      const res = await axios.get("https://brandingprofitable-29d465d7c7b1.herokuapp.com/mlm/reword", {
        params: {
      ...
      setCountData(res.data.totalRewardCount); // Make sure to adjust the key here
    } catch (error) {
