  ...
  return (
    <React.Fragment>
      ...
      >
        ...
        <TableCell
          align="center"
          onClick={() => navigate(`/allbinary_debit/${row.user_number}/${row.fullName}`)}
...
export default function BinaryPrize() {
  let navigate = useNavigate();

  const [image, setImage] = useState("");
  const [imgLoader, setImgLoader] = useState(false);

  const [binaryPrizeDebit, setBinaryPrizeDebit] = useState([]);
  ...
  const getData = async () => {
    ...
    try {
      const res = await axios.get(
        "https://brandingprofitable-29d465d7c7b1.herokuapp.com/mlm/total_binarydebit",
        {
          params: {
      ...
      setLoader(false);
      setBinaryPrizeDebit(res.data.history);
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelected = binaryPrizeDebit?.map((n) => n._id);
  ...
  let handleSearchData = async (values) => {
    let res = await axios.post(
      "https://brandingprofitable-29d465d7c7b1.herokuapp.com/mlm/search_pairincome_history",
      {
        search: values,
      }
    );
          ...
          {loader ? (
            ...
            <TableContainer>
              <Table aria-label="collapsible table">
                  ...
                  <TableRow>
                    ...
                    {/* <TableCell align="center" padding="checkbox">
                      <Checkbox
                        ...
                        indeterminate={
                          selected.length > 0 &&
                          selected.length < binaryPrizeDebit?.length
                        }
                        checked={
                          binaryPrizeDebit?.length > 0 &&
                          selected.length === binaryPrizeDebit?.length
                        }
                ...
                <TableBody>
                  {binaryPrizeDebit?.map((row, index) => {
