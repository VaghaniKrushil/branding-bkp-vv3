  ...
  return (
    <React.Fragment>
      ...
      >
        ...
        <TableCell align="center">{row.income_type}</TableCell>
...
export default function UserWalletHistory() {
  ...
  const [selectDataTableLoader, setselectDataTableLoader] = useState(false)

  const { number } = useParams();
  const getData = async () => {
    // const token = cookies.get("token");
    try {
      setselectDataTableLoader(true)
      const res = await axios.get(
        `https://brandingprofitable-29d465d7c7b1.herokuapp.com/mlm/all_userhistory/${number}/All`,
        {
          params: {
      ...
      setLoader(false);
      setselectDataTableLoader(false)
  ...
  //   if (res.data.statusCode === 200) {
  //     if (values !== "") {
  //       setTodayCreditTransaction(res.data.data);
  //     } else {
  //       getData();
  //     }
  //   }
  ...
  var filterDate = async () => {
    ...
    if (response.data.statusCode === 200) {
      setTodayCreditTransaction(response.data.history);
    ...
    <>
      ...
      <Box sx={{ width: "100%", pb: "2%", pl: "2%", pr: "2%" }}>
        ...
        <Paper sx={{ width: "100%" }} className="table_bg_img">
          ...
          {loader || dropdownLoader || searchLoader || selectDataTableLoader ? (
            <div className="d-flex flex-direction-row justify-content-center align-items-center p-5 m-5">
              <RotatingLines
                ...
                width="50"
                visible={loader || dropdownLoader || searchLoader || selectDataTableLoader}
              />
            </div>
          ) : (
            <TableContainer>
