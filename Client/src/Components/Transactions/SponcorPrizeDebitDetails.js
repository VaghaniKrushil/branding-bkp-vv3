...
export default function SponcorPrizeDebitDetails() {
  let navigate = useNavigate();
  ...
  const getData = async () => {
    ...
    try {
      const res = await axios.get(
        `https://brandingprofitable-29d465d7c7b1.herokuapp.com/mlm/total_sponsor_history/${number}`,
        {
          params: {
          ...
          >
            {selected.length > 0 ? (
              ...
              <Typography
                ...
                component="div"
              >
                Sponcor Debit History: {name}, {number}
              </Typography>
            )}
