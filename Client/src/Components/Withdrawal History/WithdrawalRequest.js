        ...
        <TableCell align="center">
          <div>
            <input
              ...
              value=""
              aria-label="..."
              checked={row.isWidrawalRequestsSendToBank == "true"}
              // disabled={true}  // Optional: To make it read-only
            />
          </div>
        </TableCell>
        ...
        <TableCell align="center">
          <div>
            <input
              ...
              value=""
              aria-label="..."
              checked={row.isWidrawalRequestsMoneyReceive == "true"}
              // disabled={true}  // Optional: To make it read-only
            />
          </div>
        </TableCell>
...
export default function WithdrawalRequest() {
  ...
  const getData = async () => {
    try {
      const res = await axios.get(
        "https://brandingprofitable-29d465d7c7b1.herokuapp.com/mlm/widrawal_pending_history",
        // "https://brandingprofitable-29d465d7c7b1.herokuapp.com/mlm/withdrawal_request",
      ...
      setCountData(res.data.totalWidrawalPendingCount); // Make sure to adjust the key here
    } catch (error) {
  ...
  let handleSearchData = async (values) => {
    let res = await axios.post(
      "https://brandingprofitable-29d465d7c7b1.herokuapp.com/mlm/widrawal_pending/search",
      {
        search: values,
      },
    );
          ...
          >
            ...
            <div className="d-flex flex-row justify-content-end gap-3 mb-3">
              {/* <img
                ...
                height={"50px"}
                src={PDFIcon}
                onClick={() => generatePDF({ allReportData })}
              ...
              {/* <img
                ...
                src={PDFIcon}
                onClick={() => {
                  generatePDF({ allReportData });
                  // Your PUT API request here
                  axios
                    .put("https://brandingprofitable-29d465d7c7b1.herokuapp.com/withdrawal/withdrawal/tobank", {
                      isWidrawalRequestsSendToBank: true,
                    })
                    .then((response) => {
                      if (response.data.statusCode === 200) {
              ...
              <img
                ...
                style={{ cursor: "pointer" }}
                onClick={async () => {
                  exportToCSV(addLanguage);

                  const transactionIds = addLanguage
                    .map((item) => item.transaction_id)
                    .join(",");

                  try {
                    const response = await axios.put(
                      "https://brandingprofitable-29d465d7c7b1.herokuapp.com/mlm/withdrawal/tobank",
                      {
