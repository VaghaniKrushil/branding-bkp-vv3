...
export default function WithdrawalSuccess() {
  ...
  const getData = async () => {
    try {
      const res = await axios.get(
        "https://brandingprofitable-29d465d7c7b1.herokuapp.com/mlm/widrawal_success_history",
      ...
      setCountData(res.data.totalWidrawalSuccessCount); // Make sure to adjust the key here
    } catch (error) {
  ...
  let handleSearchData = async (values) => {
    let res = await axios.post(
      "https://brandingprofitable-29d465d7c7b1.herokuapp.com/mlm/widrawal_success/search",
      {
        search: values,
      },
    );
