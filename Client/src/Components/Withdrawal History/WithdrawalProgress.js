...
function Row(props) {
  ...
  const handleWidrawalHistoryReject = async (row) => {
    try {
      const response = await axios.put(
        "https://brandingprofitable-29d465d7c7b1.herokuapp.com/mlm/withdrawal_reject",
        {
          ...
          reason: reason, // Ensure reason is defined and assigned somewhere in your code
          status: "fail",
        }
      );
...
export default function WithdrawalProgress() {
  ...
  const getData = async () => {
    try {
      const res = await axios.get(
        "https://brandingprofitable-29d465d7c7b1.herokuapp.com/mlm/widrawal_Progress_history",
      ...
      setCountData(res.data.totalWidrawalProgressCount); // Make sure to adjust the key here
    } catch (error) {
  ...
  let handleSearchData = async (values) => {
    let res = await axios.post(
      "https://brandingprofitable-29d465d7c7b1.herokuapp.com/mlm/widrawal_progress/search",
      {
        search: values,
      },
    );
