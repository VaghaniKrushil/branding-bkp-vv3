...
export default function WithdrawalAllHistory() {
  let navigate = useNavigate();
  ...
  const getData = async () => {
    try {
      ...
      setCountData(res.data.WithdrawalRequestCount); // Make sure to adjust the key here
    } catch (error) {
  ...
  let handleSearchData = async (values) => {
    let res = await axios.post(
      "https://brandingprofitable-29d465d7c7b1.herokuapp.com/mlm/withdrawal_request/search",
      {
        search: values,
      },
    );
