  ...
  return (
    <React.Fragment>
      ...
      >
        ...
        <TableCell align="center">
          <img
            src={imagePath + row.bannerImage}
            alt={row.bannerImage}
            width="50px"
            height="50px"
          />
        </TableCell>
        <TableCell align="center">{row.bannerLink}</TableCell>
        {/* <TableCell align="center">
          {row.bannerSwitch === true ? "True" : "False"}
        </TableCell> */}
        <TableCell align="center">
          <div>
            <Switch {...label} checked={row.bannerSwitch} />
          </div>
        </TableCell>
...
export default function MainBanner() {
  ...
  let getData = async () => {
    let res = await axios.post(
      "https://brandingprofitable-29d465d7c7b1.herokuapp.com/mainbanner/get_mainbanner",
      {
    ...
    setLoader(false);
    setMainBanner(res.data.data);
    setCountData(res.data.MainBanner);
  };
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelected = mainBanner.map((n) => n._id);
  ...
  const filterDate = async () => {
    if (data.startingDate && data.endingDate) {
      ...
      let response = await axios.post(
        "https://brandingprofitable-29d465d7c7b1.herokuapp.com/mainbanner/filterData/",
        {
      ...
      if (response.data.statusCode === 200) {
        setMainBanner(response.data.findByDateWisenData);
  ...
  let getDefaultAdvertiseBanner = async () => {
    let responce = await axios.get(
      "https://brandingprofitable-29d465d7c7b1.herokuapp.com/default_banner/main_banner_default"
    );
          ...
          <table>
            ...
            <tbody>
              {defaultAdvertiseBanner?.map((row, id) => {
                if (row.defaultBannerType === "Main Banner") {
                  return (
                    <tr key={id}>
                      <td>
                        <img
                  ...
                  <TableRow>
                    ...
                    <TableCell align="center" padding="checkbox">
                      <Checkbox
                        ...
                        indeterminate={
                          selected.length > 0 &&
                          selected.length < mainBanner?.length
                        }
                        checked={
                          mainBanner?.length > 0 &&
                          selected.length === mainBanner?.length
        ...
        <DialogContent dividers>
          <Formik
            ...
            validationSchema={Yup.object().shape({
              ...
              shedualSwitch: Yup.string("Required"),
