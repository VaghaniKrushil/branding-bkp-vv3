    ...
    return (
        <React.Fragment>
            ...
            >
                ...
                <TableCell align="center">{row.clippingAt}</TableCell>
...
export default function AddClippingCount() {
    let navigate = useNavigate();
    ...
    const getData = async () => {
        ...
        try {
            const res = await axios.get(
                "https://brandingprofitable-29d465d7c7b1.herokuapp.com/clipping/clipping",
                {
                    params: {
            ...
            setCountData(res.data.ClippingCount); // Make sure to adjust the key here
        } catch (error) {
        ...
        handleSubmit = async (values) => {
            ...
            try {
                const response = await axios.put(
                    `https://brandingprofitable-29d465d7c7b1.herokuapp.com/clipping/clipping/${id}`, // Use template literals to include the id
                    values,
                    {
                        headers: {
                            Authorization: `Bearer ${token}`,
    ...
    return (
        <>
            ...
            >
                <DialogTitle>{"Clipping Form"}</DialogTitle>
                <DialogContent dividers>
                    <Formik
                        initialValues={{
                            clippingAt:
                                editData && editData.clippingAt ? editData.clippingAt : "",
                        }}
                        validationSchema={Yup.object().shape({
                            clippingAt: Yup.number().required("Required"),
                        })}
                                ...
                                <div>
                                    <div className="mt-3">
                                        <TextField
                                            ...
                                            size="small"
                                            fullWidth
                                            placeholder="Clipping Count *"
                                            label="Clipping Count *"
                                            name="clippingAt"
                                            value={values.clippingAt}
                                            onBlur={handleBlur}
                                            onChange={handleChange}
                                        />
                                        {touched.clippingAt && errors.clippingAt ? (
                                            <div className="text-danger">{errors.clippingAt}</div>
                                        ) : null}
                                    </div>
