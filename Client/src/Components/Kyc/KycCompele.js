...
export default function KycCompele() {
  let navigate = useNavigate();

  let [kycApprovalData, setKycApprovalData] = useState([]);
  ...
  const getData = async () => {
    const token = cookies.get("token");
    try {
      const res = await axios.get("https://brandingprofitable-29d465d7c7b1.herokuapp.com/kyc/kyc_approval", {
        params: {
      ...
      setLoader(false);
      setKycApprovalData(res.data.data);
      setCountData(res.data.KycCompleteCount); // Make sure to adjust the key here
    } catch (error) {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelected = kycApprovalData?.map((n) => n._id);
  ...
  // Searchbar
    let handleSearchData = async (values) => {
      let res = await axios.post(
        "https://brandingprofitable-29d465d7c7b1.herokuapp.com/kyc/kyc_complete/search",
        {
          search: values,
        },
      );
                  ...
                  <TableRow>
                    ...
                    <TableCell align="center" padding="checkbox">
                      <Checkbox
                        ...
                        indeterminate={
                          selected.length > 0 &&
                          selected.length < kycApprovalData?.length
                        }
                        checked={
                          kycApprovalData?.length > 0 &&
                          selected.length === kycApprovalData?.length
                        }
