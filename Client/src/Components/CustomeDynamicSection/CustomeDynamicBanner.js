  ...
  return (
    <React.Fragment>
      ...
      >
        ...
        <TableCell align="center">{row.cds_bannerName}</TableCell>
        {/* <TableCell align="center">{row.bannerImage}</TableCell> */}
        <TableCell align="center">
          <img
            src={imagePath + row.cds_bannerImage}
            alt={row.cds_bannerImage}
            width="50px"
            height="50px"
          />
        </TableCell>
        <TableCell align="center">{row.cds_bannerLink}</TableCell>
        {/* <TableCell align="center">
          {row.bannerSwitch === true ? "True" : "False"}
        </TableCell> */}
        <TableCell align="center">
          <div>
            <Switch {...label} checked={row.cds_bannerSwitch} />
          </div>
        </TableCell>

        {/* <TableCell align="center">
          {row.shedualSwitch === true ? "True" : "False"}
        </TableCell> */}
        <TableCell align="center">
          <div>
            <Switch {...label} checked={row.shedualSwitch} />
          </div>
        </TableCell>
        ...
        <TableCell style={{ paddingBottom: 0, paddingTop: 0 }} colSpan={5}>
          <Collapse in={open} timeout="auto" unmountOnExit>
            <Box sx={{ paddingLeft: 15, margin: 2 }}>
              ...
              <div>
                {!row.shedualStartDate === "" ? (
                  <Typography variant="p" gutterBottom component="div">
                    <span className="fw-bold">Shedual_StartDate:</span>{" "}
                    {/* {row.shedualStartDate} */}
                    {moment(row.shedualStartDate).format("DD/MM/YYYY")}
                  </Typography>
                ) : null}

                {!row.shedualEndDate === "" ? (
                  <Typography variant="p" gutterBottom component="div">
                    <span className="fw-bold">Shedual_EndDate:</span>{" "}
                    {/* {row.shedualEndDate} */}
                    {moment(row.shedualEndDate).format("DD/MM/YYYY")}
                  </Typography>
                ) : null}
              </div>
              {/* </div> */}
...
export default function CustomeDynamicBanner() {
  let navigate = useNavigate();
  ...
  let getData = async () => {
    ...
    let res = await axios.get(
      "https://brandingprofitable-29d465d7c7b1.herokuapp.com/cd_banner/banner",
      {
        params: {
    ...
    setCountData(res.data.customeBannerCount);
  };
  ...
  let handleSearchData = async (values) => {
    const token = cookies.get("token");
    let res = await axios.post(
      "https://brandingprofitable-29d465d7c7b1.herokuapp.com/cd_banner/banner/search",
      {
        search: values,
      },
      {
  ...
  let seletedEditData = async (datas) => {
    ...
    setId(datas._id);
    setEditData(datas);
    setImage(datas.cds_bannerImage);
  };
  ...
  const filterDate = async () => {
    if (data.startingDate && data.endingDate) {
      ...
      let response = await axios.post(
        "https://brandingprofitable-29d465d7c7b1.herokuapp.com/cd_banner/filterData/",
        {
  ...
  let getDefaultAdvertiseBanner = async () => {
    let responce = await axios.get(
      "https://brandingprofitable-29d465d7c7b1.herokuapp.com/default_banner/custome_banner_default"
    );
  ...
  return (
    <>
          ...
          <table>
            ...
            <tbody>
              {defaultAdvertiseBanner?.map((row, id) => {
                if (row.defaultBannerType === "Custome Banner") {
                  return (
                    <tr key={id}>
                      <td>
                        <img
      ...
      >
        <DialogTitle>{"Custome Banner Form"}</DialogTitle>
        <DialogContent dividers>
          <Formik
            initialValues={{
              cds_bannerName:
                editData && editData.cds_bannerName
                  ? editData.cds_bannerName
                  : "",
              cds_bannerLink:
                editData && editData.cds_bannerLink
                  ? editData.cds_bannerLink
                  : "",
              cds_bannerSwitch:
                editData && editData.cds_bannerSwitch
                  ? editData.cds_bannerSwitch
                  : "",
              shedualSwitch:
            ...
            validationSchema={Yup.object().shape({
              cds_bannerName: Yup.string().required("Required"),
              //   bannerImage: Yup.string().required("Required"),
              // cds_bannerLink: Yup.string().required("Required"),
              cds_bannerSwitch: Yup.string().required("Required"),
                ...
                <div>
                  <div className="mt-3">
                    <TextField
                      ...
                      size="small"
                      fullWidth
                      placeholder="Custome-Banner *"
                      label="Custome-Banner *"
                      name="cds_bannerName"
                      value={values.cds_bannerName}
                      onBlur={handleBlur}
                      onChange={handleChange}
                    />
                    {touched.cds_bannerName && errors.cds_bannerName ? (
                      <div className="text-danger">{errors.cds_bannerName}</div>
                    ) : null}
                  </div>
                  ...
                  <div className="mb-3 mt-3">
                    ...
                    <div className="text-danger">
                      {/* Display error message when imgLoader is false */}
                      {!imgLoader && !image
                  ...
                  <div className="mt-3">
                    <TextField
                      ...
                      size="small"
                      fullWidth
                      placeholder="Custome-Banner Link *"
                      label="Custome-Banner Link *"
                      name="cds_bannerLink"
                      value={values.cds_bannerLink}
                      onBlur={handleBlur}
                      onChange={handleChange}
                    />
                  </div>
                  ...
                  <div className="w-100 mb-3 mt-3">
                    <TextField
                      ...
                      size="small"
                      select
                      label="Custome-Banner Switch"
                      name="cds_bannerSwitch"
                      value={values.cds_bannerSwitch}
                      onBlur={handleBlur}
                      onChange={handleChange}
                    >
