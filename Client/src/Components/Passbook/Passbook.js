    ...
    <React.Fragment>
      ...
      >
        ...
        <TableCell align="center">
          {" "}
          {row.C_D_type === "C" ? (
            <img src={UpArrow} alt="Credit" />
          ) : (
            <img src={DawnArrow} alt="Debit" />
          )}
        </TableCell>
      </TableRow>
    </React.Fragment>
...
export default function Passbook() {
  ...
  const getData = async () => {
    ...
    try {
      const res = await axios.get(
        "https://brandingprofitable-29d465d7c7b1.herokuapp.com/mlm/passbook",
        {
          params: {
      ...
      setCountData(res.data.totalData); // Make sure to adjust the key here
    } catch (error) {
  ...
  let handleSearchData = async (values) => {
    let res = await axios.post(
      "https://brandingprofitable-29d465d7c7b1.herokuapp.com/mlm/passbook/search",
      {
        search: values,
      },
    );
