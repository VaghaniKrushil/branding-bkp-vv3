  ...
  return (
    <React.Fragment>
      ...
      >
        ...
        <TableCell align="center">{row.popupBannerName}</TableCell>
        {/* <TableCell align="center">{row.bannerImage}</TableCell> */}
        <TableCell align="center">
          <img
            src={imagePath + row.popupBannerImage}
            alt={row.popupBannerImage}
            width="50px"
            height="50px"
          />
        </TableCell>
        ...
        {/* <TableCell align="center">
          <div>
            <Switch {...label} checked={row.popupBannerSwitch} />
          </div>
        </TableCell> */}
          ...
          <Collapse in={open} timeout="auto" unmountOnExit>
            <Box sx={{ paddingLeft: 15, margin: 2 }}>
              ...
              <div>
                ...
                <Typography variant="p" gutterBottom component="div">
                  <span className="fw-bold">Special_Date:</span>{" "}
                  {row.specialDate}
                </Typography>
              </div>
            </Box>
...
export default function PopupBanner() {
  ...
  let getData = async () => {
    let res = await axios.post(
      "https://brandingprofitable-29d465d7c7b1.herokuapp.com/popup_banner/get_popup_banner",
      {
    ...
    setCountData(res.data.PopupBannerCount);
  };
  ...
  let handleSearchData = async (values) => {
    let res = await axios.post(
      "https://brandingprofitable-29d465d7c7b1.herokuapp.com/popup_banner/search_popupbanner",
      {
        search: values,
      }
    );
  ...
  let seletedEditData = async (datas) => {
    ...
    setId(datas._id);
    setEditData(datas);
    setImage(datas.popupBannerImage);
  };
  ...
  const filterDate = async () => {
    if (data.startingDate && data.endingDate) {
      ...
      let response = await axios.post(
        "https://brandingprofitable-29d465d7c7b1.herokuapp.com/popup_banner/filterData/",
        {
...
if (!defaultBannerId) {
  ...
  let getDefaultAdvertiseBanner = async () => {
    let responce = await axios.get(
      "https://brandingprofitable-29d465d7c7b1.herokuapp.com/default_banner/popup_banner_default"
    );
          ...
          <table>
            ...
            <tbody>
              {defaultAdvertiseBanner?.map((row, id) => {
                if (row.defaultBannerType === "Popup Banner") {
                  return (
                    <tr key={id}>
                      <td>
                        <img
        ...
        <DialogContent dividers>
          <Formik
            ...
            validationSchema={Yup.object().shape({
              ...
              shedualStartDate: Yup.string().required("Required"),
              shedualEndDate: Yup.string().required("Required"),
            })}
