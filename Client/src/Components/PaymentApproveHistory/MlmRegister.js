...
export default function MlmRegister() {
  ...
  const getData = async () => {
    try {
      const res = await axios.get(
        "https://brandingprofitable-29d465d7c7b1.herokuapp.com/mlm/payment_check",
        {
          params: {
      ...
      setCountData(res.data.MlmPendingCount); // Make sure to adjust the key here
    } catch (error) {
  ...
  function Row(props) {
    const { row, handleClick, isItemSelected, labelId, seletedEditData } =
      props;
    ...
    const handleKycSubmit = async (mobileNumber, status) => {
      try {
        // setLoading(true); 
        ...
        // data.employeeRegisterDate 
        //   .add(1, "seconds")
        //   .format("YYYY-MM-DD HH:mm:ss");
        const response = await axios.put(
          `https://brandingprofitable-29d465d7c7b1.herokuapp.com/mlm/payment_check/${mobileNumber}`,
          data
        );
        ...
        >
          ...
          <TableCell align="center">
            <a href={row.image} target="_blank" rel="noopener noreferrer">
              <img src={row.image} alt={row.image} width="50px" height="50px" />
            </a>
          </TableCell>
          ...
          <TableCell align="center">{row.referredBy}</TableCell>
          <TableCell align="center">{row.treeId}</TableCell>
                ...
                <div className="d-flex justify-content-center gap-3">
                  {showDeniedReason ? (
                    ...
                    <div className="d-flex justify-content-center gap-3">
                      <div>
                        <Button
                          onClick={() =>
                            handleKycSubmit(row.mobileNumber, "Complete")
                          }
                          // disabled={loading}
                        >
                           {/* {loading ? 'Approving...' : 'Approval'} */}
                          Approval
                        </Button>
                      </div>
                      <div>
                        <Button 
                        onClick={() => setShowDeniedReason(true)}
                        // disabled={loading}
                        >
                  ...
                  {showDeniedReason && (
                    <div className="mt-3">
                      <Button
                        onClick={() =>
                          handleKycSubmit(row.mobileNumber, "Reject")
                        }
                        // disabled={loading}
                      >
  ...
  let handleSearchData = async (values) => {
    let res = await axios.post(
      "https://brandingprofitable-29d465d7c7b1.herokuapp.com/mlm/payment_pending/search",
      {
        search: values,
      },
    );
