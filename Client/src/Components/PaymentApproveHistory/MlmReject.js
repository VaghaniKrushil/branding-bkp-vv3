...
export default function MlmReject() {
  ...
  const getData = async () => {
    try {
      const res = await axios.get(
        "https://brandingprofitable-29d465d7c7b1.herokuapp.com/mlm/payment_reject",
        {
          params: {
      ...
      setCountData(res.data.MlmRejectCount); // Make sure to adjust the key here
    } catch (error) {
  ...
  let handleSearchData = async (values) => {
    let res = await axios.post(
      "https://brandingprofitable-29d465d7c7b1.herokuapp.com/mlm/payment_reject/search",
      {
        search: values,
      },
    );
