...
export default function MlmComplete() {
  ...
  const getData = async () => {
    try {
      const res = await axios.get("https://brandingprofitable-29d465d7c7b1.herokuapp.com/mlm/payment_complete", {
        params: {
      ...
      setCountData(res.data.MlmCompleteCount); // Make sure to adjust the key here
    } catch (error) {
  ...
  var handleDelete = () => {
    const token = cookies.get("token");
    axios
      .delete("https://brandingprofitable-29d465d7c7b1.herokuapp.com/language/language", {
        data: selected,
        headers: {
  ...
  let handleSearchData = async (values) => {
    const token = cookies.get("token");
    let res = await axios.post(
      "https://brandingprofitable-29d465d7c7b1.herokuapp.com/mlm/payment_complete/search",
      {
        search: values,
      },
    );
