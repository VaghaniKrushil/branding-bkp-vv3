  ...
  return (
    <React.Fragment>
      ...
      >
        ...
        <TableCell align="center">{row.notificationContent}</TableCell>
...
export default function BusinessNotification() {
  ...
  const getData = async () => {
    ...
    try {
      const res = await axios.get(
        "https://brandingprofitable-29d465d7c7b1.herokuapp.com/notification/notification",
        {
          params: {
  ...
  var handleDelete = () => {
    swal("Are You Sure You Want TO Delete ?", {
      ...
      if (buttons === true) {
        axios
          .delete("https://brandingprofitable-29d465d7c7b1.herokuapp.com/notification/notification", {
            data: selected,
          })
          .then((response) => {
  ...
  let [
    modalShowForPopupPersonalNotificationForm,
    setModalShowForPopupPersonalNotificationForm,
  ...
  var handleSubmitForBusinessNotification;

  if (!id) {
    handleSubmitForBusinessNotification = async (values) => {
      const token = cookies.get("token"); // Replace with your actual JWT token
      try {
        const res = await axios.post(
          "https://brandingprofitable-29d465d7c7b1.herokuapp.com/notification/notification/business",
          values,
          {
            headers: {
              Authorization: `Bearer ${token}`,
  ...
  return (
    <>
          ...
          <div className="d-flex flex-row justify-content-end mb-2 gap-2">
            <>
              ...
              <Button
                ...
                size="small"
                onClick={() => {
                  setModalShowForPopupPersonalNotificationForm(true);
                  setId(null);
                  setEditData({});
                }}
              >
      ...
      >
        <DialogTitle>{"Notification Form"}</DialogTitle>
        <DialogContent dividers>
          <Formik
            initialValues={{
              notificationTitle:
                editData && editData.notificationTitle
                  ? editData.notificationTitle
                  : "",
              notificationContent:
                editData && editData.notificationContent
                  ? editData.notificationContent
                  : "",
              // isPersonal:
              //   editData && editData.isPersonal ? editData.isPersonal : "",
            }}
                ...
                <div>
                  ...
                  {/* <div className="w-100 mb-3 mt-3">
                    <TextField
                      ...
                      label="Select Value"
                      name="isPersonal"
                      value={values.isPersonal}
                      onBlur={handleBlur}
                      onChange={handleChange}
                    >
                      <MenuItem value={true}>Personal</MenuItem>
                      <MenuItem value={false}>Business</MenuItem>
                    </TextField>
                    {touched.isPersonal && errors.isPersonal ? (
                      <div className="text-danger">{errors.isPersonal}</div>
                    ) : null}
                  </div> */}
      ...
      <Dialog
        fullWidth
        open={modalShowForPopupPersonalNotificationForm}
        onClose={() => setModalShowForPopupPersonalNotificationForm(false)}
      >
        <DialogTitle>{"Notification Form"}</DialogTitle>
        <DialogContent dividers>
          <Formik
            ...
            onSubmit={(values, { resetForm }) => {
              handleSubmitForBusinessNotification(values);
              resetForm(values);
            }}
          >
            {({ values, errors, touched, handleBlur, handleChange }) => (
