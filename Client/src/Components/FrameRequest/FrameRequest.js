...
function Row(props) {
  ...
  const handleDownload = () => {
    fetch(row.frameRequestImage)
      .then((res) => res.blob())
      .then((blob) => {
        saveAs(blob, "FrameRequest_image");
      });
  };
    ...
    <React.Fragment>
      ...
      >
        ...
        <TableCell align="center">
          {moment(row.frameRequestDate).format("DD/MM/YYYY")}
        </TableCell>

        {/* <TableCell align="center">
          <img
            src={row.advertiseImage}
            alt={row.advertiseImage}
            width="50px"
            height="50px"
          />
        </TableCell> */}
        <TableCell align="center">
          {row.isFrameCreated ? "Complete" : "Pending"}
        </TableCell>
        {/* <TableCell align="center">
          <img
            src={ row.frameRequestImage}
            alt={row.frameRequestImage}
            width="50px"
            height="50px"
          />
        </TableCell> */}
        ...
        <TableCell align="center">{row.frameRequestMessage}</TableCell>

        <TableCell align="center">
          <button
            class="btn btn-default"
            onClick={() => navigate("/canva-userframe")}
          >
            <AddIcon />
          </button>
        </TableCell>
...
export default function FrameRequest() {
  let navigate = useNavigate();
  ...
  // let [advertiseBanner, setadvertiseBanner] = useState([]);
  let [advertiseBanner, setAdvertiseBanner] = useState([]);
  ...
  //     "https://brandingprofitable-29d465d7c7b1.herokuapp.com/framerequest/framerequest",
  //     {
  //       params: {
  ...
  //   );
  //   setLoader(false);
  //   setAdvertiseBanner(res.data.data);
  //   setCountData(res.data.AdvertiseBannerCount);
  // };

  let getData = async () => {
    let res = await axios.post(
      "https://brandingprofitable-29d465d7c7b1.herokuapp.com/framerequest/framerequest/get",
      {
        pageSize: rowsPerPage,
        pageNumber: page,
      }
    ...
    setLoader(false);
    setAdvertiseBanner(res.data.data);
    setCountData(res.data.FrameRequestCount);
  };
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelected = advertiseBanner.map((n) => n._id);
  ...
  var handleDelete = () => {
    axios
      .delete("https://brandingprofitable-29d465d7c7b1.herokuapp.com/framerequest/framerequest", {
        data: selected,
      })
      .then((response) => {
  ...
  let handleSearchData = async (values) => {
    let res = await axios.post(
      "https://brandingprofitable-29d465d7c7b1.herokuapp.com/framerequest/framerequest/search",
      {
        search: values,
      }
    );
  ...
  if (!id) {
    handleSubmit = async (values) => {
      const token = cookies.get("token");
      values["advertiseImage"] = image;
      let res = await axios.post(
        "https://brandingprofitable-29d465d7c7b1.herokuapp.com/advertise_banner/banner",
        values,
        {
          headers: {
            Authorization: `Bearer ${token}`,
    ...
    handleSubmit = async (values) => {
      ...
      let response = await axios.put(
        "https://brandingprofitable-29d465d7c7b1.herokuapp.com/advertise_banner/banner/" + id,
        values,
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const filterDate = async () => {
    if (data.startingDate && data.endingDate) {
      ...
      const endingDateFormatted = moment(data.endingDate).format("YYYY-MM-DD");

      let response = await axios.post(
        "https://brandingprofitable-29d465d7c7b1.herokuapp.com/framerequest/filterData/",
        {
      ...
      if (response.data.statusCode === 200) {
        setAdvertiseBanner(response.data.findByDateWisenData);
  ...
  return (
    <>
                  ...
                  <TableRow>
                    <TableCell align="center" padding="checkbox">
                      <Checkbox
                        ...
                        indeterminate={
                          selected.length > 0 &&
                          selected.length < advertiseBanner?.length
                        }
                        checked={
                          advertiseBanner?.length > 0 &&
                          selected.length === advertiseBanner?.length
                        }
      ...
      >
        <DialogTitle>{"Advertise Banner Form"}</DialogTitle>
        <DialogContent dividers>
          <Formik
            initialValues={{
              advertiseBannerName:
                editData && editData.advertiseBannerName
                  ? editData.advertiseBannerName
                  : "",
              advertiseLink:
                editData && editData.advertiseLink
                  ? editData.advertiseLink
                  : "",
              advertiseSwitch:
                editData && editData.advertiseSwitch
                  ? editData.advertiseSwitch
                  : "",
              shedualSwitch:
            ...
            validationSchema={Yup.object().shape({
              advertiseBannerName: Yup.string().required("Required"),
              //   bannerImage: Yup.string().required("Required"),
              advertiseLink: Yup.string().required("Required"),
              advertiseSwitch: Yup.string().required("Required"),
              shedualSwitch: Yup.string().required("Required"),
                ...
                <div>
                  <div className="mt-3">
                    <TextField
                      ...
                      size="small"
                      fullWidth
                      placeholder="Advertise-Banner Name *"
                      label="Advertise-Banner Name *"
                      name="advertiseBannerName"
                      value={values.advertiseBannerName}
                      onBlur={handleBlur}
                      onChange={handleChange}
                    />
                    {touched.advertiseBannerName &&
                    errors.advertiseBannerName ? (
                  ...
                  <div className="mb-3 mt-3">
                    <label>Advertise-Banner Image</label>
                    <input
                      type="file"
                  ...
                  <div className="mt-3">
                    <TextField
                      ...
                      size="small"
                      fullWidth
                      placeholder="Advertise-Banner Link *"
                      label="Advertise-Banner Link *"
                      name="advertiseLink"
                      value={values.advertiseLink}
                      onBlur={handleBlur}
                      onChange={handleChange}
                    />
                    {touched.advertiseLink && errors.advertiseLink ? (
                      <div className="text-danger">{errors.advertiseLink}</div>
                    ) : null}
                  </div>
                  ...
                  <div className="w-100 mb-3 mt-3">
                    <TextField
                      ...
                      size="small"
                      select
                      label="Advertise-Banner Switch"
                      name="advertiseSwitch"
                      value={values.advertiseSwitch}
                      onBlur={handleBlur}
                      onChange={handleChange}
                    >
