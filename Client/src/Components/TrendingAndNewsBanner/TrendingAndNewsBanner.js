...
// function TrendingAndNewsBanner() {
//   return (
//     <div>TrendingAndNewsBanner</div>
//   )
// }

// export default TrendingAndNewsBanner
  ...
  return (
    <React.Fragment>
      ...
      >
        ...
        <TableCell align="center">{row.trendingBannerName}</TableCell>
        {/* <TableCell align="center">{row.bannerImage}</TableCell> */}
        <TableCell align="center">
          <img
            src={imagePath + row.trendingBannerImage}
            alt={row.trendingBannerImage}
            width="50px"
            height="50px"
          />
        </TableCell>
        <TableCell align="center">{row.shedualDate}</TableCell>
        <TableCell align="center">
          <div>
            <Switch {...label} checked={row.trendingBannerSwitch} />
          </div>
        </TableCell>
...
export default function PopupBanner() {
  ...
  let getData = async () => {
    let res = await axios.post(
      "https://brandingprofitable-29d465d7c7b1.herokuapp.com/trending/get_trending_banner",
      {
  ...
  var handleDelete = () => {
    axios
      .post("https://brandingprofitable-29d465d7c7b1.herokuapp.com/trending/delete_popup_banner", selected)
      .then((response) => {
  ...
  let handleSearchData = async (values) => {
    let res = await axios.post(
      "https://brandingprofitable-29d465d7c7b1.herokuapp.com/trending/search_trending_banner",
      {
        search: values,
      }
    );
  ...
  if (!id) {
    var handleSubmit = async (values) => {
      ...
      values["trendingBannerImage"] = image;
      let res = await axios.post(
        "https://brandingprofitable-29d465d7c7b1.herokuapp.com/trending/trending_banner",
        values
      );
      if (res.data.statusCode === 200) {
    ...
    handleSubmit = async (values) => {
      ...
      let response = await axios.put(
        "https://brandingprofitable-29d465d7c7b1.herokuapp.com/trending/trending_banner/" + id,
        values
      );
  ...
  return (
    <>
      ...
      >
        <DialogTitle>{"Trending-Banner Form"}</DialogTitle>
        <DialogContent dividers>
          <Formik
            initialValues={{
              trendingBannerName:
                editData && editData.trendingBannerName
                  ? editData.trendingBannerName
                  : "",
              //   bannerImage:
              //     editData && editData.bannerImage ? editData.bannerImage : "",
              trendingBannerSwitch:
                editData && editData.trendingBannerSwitch
                  ? editData.trendingBannerSwitch
                  : "",
              shedualDate:
                editData && editData.shedualDate ? editData.shedualDate : "",
            }}
            validationSchema={Yup.object().shape({
              trendingBannerName: Yup.string().required("Required"),
              trendingBannerSwitch: Yup.boolean().required("Required"),
                ...
                <div>
                  <div className="mt-3">
                    <TextField
                      ...
                      size="small"
                      fullWidth
                      placeholder="Trending-Banner Name *"
                      label="Trending-Banner Name *"
                      name="trendingBannerName"
                      value={values.trendingBannerName}
                      onBlur={handleBlur}
                      onChange={handleChange}
                    />
                    {touched.trendingBannerName && errors.trendingBannerName ? (
                      <div className="text-danger">
                        {errors.trendingBannerName}
                      </div>
                    ) : null}
                  </div>
                  ...
                  <div className="w-100 mb-3 mt-3">
                    <TextField
                      ...
                      select
                      label="Trending-Banner Switch"
                      name="trendingBannerSwitch"
                      value={values.trendingBannerSwitch}
                      onBlur={handleBlur}
                      onChange={handleChange}
                    >
                    ...
                    {touched.trendingBannerSwitch &&
                    errors.trendingBannerSwitch ? (
                      <div className="text-danger">
                        {errors.trendingBannerSwitch}
                      </div>
                    ) : null}
