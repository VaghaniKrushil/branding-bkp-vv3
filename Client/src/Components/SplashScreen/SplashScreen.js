...
function Row(props) {
  ...
  // -------------------------------Splash On/Off Switch---------------------------------

  const handleToggle = (id) => {
    const apiUrl = `https://brandingprofitable-29d465d7c7b1.herokuapp.com/splashscreen/splashscreen_onoff/${id}`;
  ...
  return (
    <React.Fragment>
      ...
      >
        ...
        <TableCell align="center">{row.splashScreenName}</TableCell>

        <TableCell align="center">
          <img
            src={imagePath + row.splashScreenLogo}
            alt={row.splashScreenLogo}
            width="50px"
            height="50px"
          />
        </TableCell>
        ...
        <TableCell align="center">
          {moment(row.specialDate).format("DD/MM/YYYY")}
        </TableCell>
        ...
        {/* <TableCell align="center">
          <div>
            <Switch {...label} checked={row.isSplashScreenDaysSwitch} />
          </div>
        </TableCell> */}

        <TableCell align="center">{row.showSplashScreenDays}</TableCell>
...
export default function SplashScreen() {
  ...
  const fileData = (file) => {
    ...
    if (fileSizeInBytes > maxSizeInBytes) {
      swal("", "File size exceeds the maximum limit of 10MB", "error");
      return;
    }
  ...
  const getData = async () => {
    try {
      const res = await axios.get(
        "https://brandingprofitable-29d465d7c7b1.herokuapp.com/splashscreen/splashscreen/get",
        {
          params: {
      ...
      setLoader(false);
      setSplashScreen(res.data.data);
      setCountData(res.data.SplashScreenCount); // Make sure to adjust the key here
    } catch (error) {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelected = splashScreen.map((n) => n._id);
  ...
  var handleDelete = () => {
    swal("Are You Sure You Want TO Delete ?", {
      ...
      if (buttons === true) {
        axios
          .delete("https://brandingprofitable-29d465d7c7b1.herokuapp.com/splashscreen/splashscreen/remove", {
            data: selected,
          })
          .then((response) => {
  ...
  if (!id) {
    var handleSubmit = async (values) => {
      ...
      values["splashScreenLogo"] = image;
      // values["showCategoryDaysSwitch"] = switchChecked;
      values["isSplashScreenDaysSwitch"] = switchChecked;
      let res = await axios.post(
        "https://brandingprofitable-29d465d7c7b1.herokuapp.com/splashscreen/splashscreen",
        values
      );
      if (res.data.statusCode === 200) {
    ...
    handleSubmit = async (values) => {
      ...
      let response = await axios.put(
        "https://brandingprofitable-29d465d7c7b1.herokuapp.com/splashscreen/splashscreen/" + id,
        values
      );
  ...
  let seletedEditData = async (datas) => {
    ...
    setId(datas._id);
    setEditData(datas);
    setImage(datas.splashScreenLogo);
    setSwitchChecked(datas.isSplashScreenDaysSwitch);
    console.log(datas);
  };
  ...
  var filterDate = async () => {
    if (data.startingDate && data.endingDate) {
      ...
      if (response.data.statusCode === 200) {
        setSplashScreen(response.data.findByDateWisenData);
  ...
  const [defaultSplashScreen, setDefaultSplashScreen] = useState();

  let getdefaultSplashScreen = async () => {
    let responce = await axios.get(
      "https://brandingprofitable-29d465d7c7b1.herokuapp.com/default_banner/splash_screen_default"
    );
    setDefaultSplashScreen(responce.data.data);
  };
  ...
  return (
    <>
      ...
      <Box sx={{ width: "100%", pb: "2%", pl: "2%", pr: "2%" }}>
        ...
        <div className="shadowed-div border rounded p-3 mb-3">
          <table>
            ...
            <tbody>
              {defaultSplashScreen?.map((row, id) => {
                if (row.defaultBannerType === "Splash Screen") {
                  return (
                    <tr key={id}>
                      <td>
                        <img
                  ...
                  <TableRow>
                    ...
                    <TableCell align="center" padding="checkbox">
                      <Checkbox
                        ...
                        indeterminate={
                          selected.length > 0 &&
                          selected.length < splashScreen?.length
                        }
                        checked={
                          splashScreen?.length > 0 &&
                          selected.length === splashScreen?.length
                        }
      ...
      >
        <DialogTitle>{"Splash Screen Form"}</DialogTitle>
        <DialogContent dividers>
          <Formik
            initialValues={{
              ...
              splashScreenName:
                editData && editData.splashScreenName
                  ? editData.splashScreenName
                  : "",
              isSplashScreenDaysSwitch:
                editData && editData.isSplashScreenDaysSwitch
                  ? editData.isSplashScreenDaysSwitch
                  : "",
              showSplashScreenDays:
                editData && editData.showSplashScreenDays
                  ? editData.showSplashScreenDays
                  : "",
            }}
            validationSchema={Yup.object().shape({
              specialDate: Yup.string().required("Required"),
              splashScreenName: Yup.string().required("Required"),
            })}
                ...
                <div>
                  <div className="mt-3">
                    <TextField
                      ...
                      size="small"
                      fullWidth
                      placeholder="Splash-Screen Name *"
                      label="Splash-Screen Name *"
                      name="splashScreenName"
                      value={values.splashScreenName}
                      onBlur={handleBlur}
                      onChange={handleChange}
                    />
                    {touched.splashScreenName && errors.splashScreenName ? (
                      <div className="text-danger">
                        {errors.splashScreenName}
                      </div>
                    ) : null}
                  </div>
                    ...
                    {switchChecked === false ? (
                      <div>
                        <TextField
                          ...
                          label="Enter Day *"
                          name="showSplashScreenDays"
                          value={values.showSplashScreenDays}
                          onBlur={handleBlur}
                          onChange={handleChange}
                        />
                      </div>
