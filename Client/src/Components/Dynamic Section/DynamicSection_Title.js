  ...
  return (
    <React.Fragment>
      ...
      >
        ...
        <TableCell align="center">
          <img
            src={imagePath + row.ds_image}
            alt={imagePath + row.ds_image}
            width="50px"
            height="50px"
          />
        </TableCell>
        ...
        <TableCell align="center">
          <div>
            <Switch {...label} checked={row.ds_switch} />
          </div>
        </TableCell>
...
export default function DynamicSection_Title() {
  let navigate = useNavigate();
  ...
  let getData = async () => {
    let res = await axios.post(
      "https://brandingprofitable-29d465d7c7b1.herokuapp.com/ds_title/get_ds_title",
      {
    ...
    setCountData(res.data.DynamicSection_TitleCount);
  };
  ...
  var handleDelete = () => {
    swal("Are You Sure You Want TO Delete ?", {
      ...
      if (buttons === true) {
        axios
          .post(
            "https://brandingprofitable-29d465d7c7b1.herokuapp.com/ds_title/delete_ds_title",
            selected
          )
          .then((response) => {
  ...
  let handleSearchData = async (values) => {
    let res = await axios.post(
      "https://brandingprofitable-29d465d7c7b1.herokuapp.com/ds_title/search_ds_category",
      {
        search: values,
      }
    );
  ...
  if (!id) {
    var handleSubmit = async (values) => {
      ...
      values["ds_image"] = image;
      values["comp_iamge"] = compImage;
      values["addDate"] = moment()
        .add(2, "seconds")
        .format("YYYY-MM-DD HH:mm:ss");
      let response = await axios.post(
        "https://brandingprofitable-29d465d7c7b1.herokuapp.com/ds_title/ds_title",
        values
      );
      if (response.data.statusCode === 200) {
  ...
  const filterDate = async () => {
    if (data.startingDate && data.endingDate) {
      ...
      let response = await axios.post(
        "https://brandingprofitable-29d465d7c7b1.herokuapp.com/ds_title/filterData/",
        {
    ...
    <>
      ...
      <Box sx={{ width: "100%", pb: "2%", pl: "2%", pr: "2%" }}>
        <div id="main-btn-add-machinetype">
          ...
          <div className="d-flex justify-content-end gap-2 m-3 mb-3">
            <TextField
        ...
        <DialogContent dividers>
          <Formik
            initialValues={{
              ...
              ds_switch:
                editData && editData.ds_switch ? editData.ds_switch : "",
            }}
            validationSchema={Yup.object().shape({
              ds_category: Yup.string().required("Required this field"),
              ds_switch: Yup.boolean().required("Required this field"),
            })}
