...
const headCells = [
  ...
  {
    label: "Designation/BusinessType",
  },
  {
    label: "Mobile Number",
  },
...
function Row(props) {
  ...
  function getRowImageUrl(row) {
    const currentCdn = "https://www.sparrowgroups.com";
    const newCdn = "https://cdn.dohost.in/upload/";
  
    // Check if the image URL contains the current CDN
    if (row.profileImage && row.profileImage.includes(currentCdn)) {
      // Replace the current CDN with the new CDN
      const imageName = row.profileImage.replace(currentCdn + "/CDN/upload/", "");
      return newCdn + imageName;
    }
  
    // If the current CDN is not present, use the existing CDN
    return row.profileImage || row.businessLogo;
  }
  ...
  return (
    <React.Fragment>
      ...
      >
        ...
        {/* <TableCell align="center">
          <a href={row.profileImage || row.businessLogo} target="_blank" rel="noopener noreferrer">
            <img
              src={row.profileImage || row.businessLogo}
              alt={row.profileImage || row.businessLogo}
              width="50px"
              height="50px"
            />
          </a>
...
<TableCell align="center">
  <a href={getRowImageUrl(row)} target="_blank" rel="noopener noreferrer">
    <img
    ...
    />
        ...
        {/* <TableCell align="center">
          {(row.profileImage || row.businessLogo) &&
          !row.profileImage?.includes(
            "https://www.sparrowgroups.com/CDN/upload/"
          ) &&
          !row.businessLogo?.includes(
            "https://www.sparrowgroups.com/CDN/upload/"
          ) ? (
            <a
              href={row.profileImage || row.businessLogo}
              target="_blank"
        ...
        <TableCell align="center">{row.fullName || row.businessLogo}</TableCell>
        <TableCell align="center">
          {row.Designation || row.businessType}
        </TableCell>
...
export default function NormalUsers() {
  ...
  const getData = async () => {
    ...
    try {
      const res = await axios.get(
        "https://brandingprofitable-29d465d7c7b1.herokuapp.com/user/users",
        {
          params: {
      ...
      setCountData(res.data.UsersCount); // Make sure to adjust the key here
    } catch (error) {
  ...
  let handleSearchData = async (values) => {
    ...
    const token = cookies.get("token");
    let res = await axios.post(
      "https://brandingprofitable-29d465d7c7b1.herokuapp.com/user/search_user",
      {
        search: values,
      },
      {
